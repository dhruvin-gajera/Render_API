...
def create_competition_entry(
    ...
    competition = (
        ...
        .filter(models.Competition.id == entry.competition_id)
        .first()
    )
    if not competition:
        raise HTTPException(status_code=404, detail="Competition not found")

    # Check if the user is already participating in a competition
    existing_entry = (
        db.query(models.CompetitionEntry)
        .filter(models.CompetitionEntry.user_id == entry.user_id, 
                models.CompetitionEntry.is_deleted == False)  # Exclude soft-deleted entries
        .first()
    )
    if existing_entry:
        raise HTTPException(
            status_code=400,
            detail=f"User is already participating in another competition: {existing_entry.competition_id}",
        )

    # Check if the competition is full
    current_entries_count = (
        db.query(models.CompetitionEntry)
        .filter(models.CompetitionEntry.competition_id == entry.competition_id)
        .count()
    )
    if current_entries_count >= competition.user_capacity:
        raise HTTPException(status_code=400, detail="Competition is full")

    # Create the new competition entry
    ...
    empty_positions = competition.user_capacity - current_entries_count - 1
